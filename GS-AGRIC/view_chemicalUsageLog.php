           ...
           if(r){ 
  			...
  			success: function(data) {
    			window.location ='view_purchase.php';
                        ...
                        jAlert('Record Is Deleted', 'POSNIC');
  			}
		});
            }
            return r;
			...
			<div class="side-content fr">
				...
				<div class="content-module">
					...
					<div class="content-module-heading cf">
						...
						<h3 class="fl">Chemical Usage Log</h3>
...
<input name="dsubmit" type="button" value="Delete Selected" class="my_button round blue   text-upper" style="margin-left:5px;" onclick="return confirmDeleteSubmit()"/>
	<input name="showAll" type="button" value="Print All" class="my_button round green text-upper"	style="margin-left:5px;" />	
...
$SQL = "SELECT DISTINCT(chemid) FROM  chemicallog";

if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{

$SQL = "SELECT DISTINCT(chemid)FROM chemicallog WHERE chemid LIKE '%".$_POST['searchtxt']."%'";
	...
	*/
	...
	$query = "SELECT COUNT(DISTINCT chemid) as num FROM $tbl_name";
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{

$query = "SELECT COUNT(DISTINCT chemid) as num FROM chemicallog WHERE chemName LIKE '%".$_POST['searchtxt']."%' ";
	...
	$targetpage = "view_chemicalUsageLog.php"; 	//your file name  (the name of this file)
...
$sql = "SELECT * FROM chemicallog WHERE name LIKE '%".$_POST['searchtxt']."%'  LIMIT $start, $limit";
	...
	*/
	...
	{	
		...
		if ($page > 1) 
			...
			$pagination.= "<a href=\"view_chemicalUsageLog.php?page=$prev&limit=$limit\" class=my_pagination >Previous</a>";
...

		{
...

			{
				...
				$pagination.= "<a href=\"view_chemicalUsageLog.php?page=1&limit=$limit\" class=my_pagination>1</a>";

				$pagination.= "<a href=\"view_chemicalUsageLog.php?page=2&limit=$limit\" class=my_pagination>2</a>";
...

				{
					...
					if ($counter == $page)
						...
						$pagination.= "<a href=\"view_chemicalUsageLog.php?page=$counter&limit=$limit\" class=my_pagination>$counter</a>";					

				}

				$pagination.= "...";

				$pagination.= "<a href=\"view_chemicalUsageLog.php?page=$lpm1&limit=$limit\" class=my_pagination>$lpm1</a>";

				$pagination.= "<a href=\"view_chemicalUsageLog.=$lastpage&limit=$limit\" class=my_pagination>$lastpage</a>";		
		...
		if ($page < $counter - 1) 
			...
			$pagination.= "<a href=\"view_chemicalUsageLog.php?page=$next&limit=$limit\" class=my_pagination>Next</a>";
...
while($row = mysqli_fetch_array($result)) 
{
 ...
 <td> <?php echo $row['remark']; ?></td>
    ...
    <td><a  href=print_chemical_log.php?id="<?php echo $row['id'];?>" target="_blank">print</a>
    </td>
...
<?php
							...
							{
								$sql= mysqli_query($conn,"SELECT * FROM chemicallog");

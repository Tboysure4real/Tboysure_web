...
{ 	     jConfirm('You Want Delete This scaleCalibration ', 'Confirmation Dialog', function (r) {
           if(r){ 
               console.log();
                $.ajax({
  			url: "delete.php",
  			...
  			success: function(data) {
    			window.location ='view_scaleCalibration.php';
...
return false;
 ...
 jConfirm('You Want Delete scaleCalibration', 'Confirmation Dialog', function (r) {
           if(r){ 
			...
			<div class="side-content fr">
				...
				<div class="content-module">
					...
					<div class="content-module-heading cf">
						...
						<h3 class="fl">Scale Calibration Report</h3>
...
<input type="hidden" name="table" value="scaleCalibration">
<input type="hidden" name="return" value="view_scaleCalibration.php">
...
$SQL = "SELECT DISTINCT(secId) FROM  scalecalibration";

if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{

$SQL = "SELECT DISTINCT(secId)FROM  scalecalibration WHERE secId LIKE '%".$_POST['searchtxt']."%'";

}

	$tbl_name="scalecalibration";		//your table name
	...
	*/
	...
	$query = "SELECT COUNT(DISTINCT secId) as num FROM $tbl_name";
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{

$query = "SELECT COUNT(DISTINCT slauId) as num FROM scalecalibrCation WHERE name LIKE '%".$_POST['searchtxt']."%' ";
	...
	$targetpage = "view_scaleCalibration.php"; 	//your file name  (the name of this file)
...
$sql = "SELECT * FROM scaleCalibration WHERE name LIKE '%".$_POST['searchtxt']."%'  LIMIT $start, $limit";
	...
	*/
	...
	{	
		...
		if ($page > 1) 
			...
			$pagination.= "<a href=\"view_scaleCalibration.php?page=$prev&limit=$limit\" class=my_pagination >Previous</a>";
		...
		{	
...

			{
				...
				if ($counter == $page)
					...
					$pagination.= "<a href=\"view_scaleCalibration.php?page=$counter&limit=$limit\" class=my_pagination>$counter</a>";					
...

		{
...

			{
				...
				$pagination.= "<a href=\"view_scaleCalibration.php?page=$lpm1&limit=$limit\" class=my_pagination>$lpm1</a>";

				$pagination.= "<a href=\"view_scaleCalibration.php?page=$lastpage&limit=$limit\" class=my_pagination>$lastpage</a>";		
...

			{
				...
				$pagination.= "<a href=\"view_scaleCalibration.php?page=1&limit=$limit\" class=my_pagination>1</a>";

				$pagination.= "<a href=\"view_scaleCalibration.php?page=2&limit=$limit\" class=my_pagination>2</a>";
		...
		if ($page < $counter - 1) 
			...
			$pagination.= "<a href=\"view_scaleCalibration.php?page=$next&limit=$limit\" class=my_pagination>Next</a>";
...
while($row = mysqli_fetch_array($result))
...

		{
			...
			$line = $db->queryUniqueObject("SELECT * FROM scaleCalibration");
 ...
 ?> 
  ...
  <td> <?php echo $row['callTime3']; ?></td>
   ...
   <td> <?php echo $row['manager']; ?></td>
    ...
    <td><a  href=print_scaleCalibration_report.php?id="<?php echo $row['id'];?>" target="_blank">print</a>
    </td>
